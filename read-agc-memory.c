...
int ParseIoPacket(unsigned char *Packet, int *Channel, int *Value, int *uBit)
{
...
void DisplayRegister(char *registerdigits)
{
    ...
    char newoctalstring[16];

    sprintf(newoctalstring, "%o%o%o%o%o",
            registerdigits[0],
            registerdigits[1],
...
void ActOnIncomingIO(unsigned char *Packet)
{
    ...
    if (Packet != NULL)
    {
        ...
        // need to check it.
        if (0x40 != (Packet[1] & 0xc0) || 0x80 != (Packet[2] & 0xc0) || 0xc0 != (Packet[3] & 0xc0))
            return;
...
int makeConnection()
{
    ...
    if (ServerSocket == -1)
    {
        ServerSocket = CallSocket(DefaultHostname, Portnum);
        if (ServerSocket != -1)
...
int main(int argc, char *argv[])
{
    ...
    if (argc < 4)
    {
        printf("%s [ip or hostname] [port] [mem value]\n", argv[0]);
        return 0;
    }

    int len = strlen(argv[1]);
    strncpy(DefaultHostname, argv[1], sizeof(DefaultHostname) < len + 1 ? sizeof(DefaultHostname) : len);
    Portnum = (unsigned short)atoi(argv[2]);

    strncpy(memtoread_str, argv[3], 6);
    printf("%s\n", memtoread_str);
